...
**已确证，且是目前找到的最大杠杆——关闭模型的 reasoning 阶段。** `deepseek-flash` 每次作答前都先思考，而 agent 循环每一轮都要付这笔钱。受控实验：同端点、同任务、同 prompt，唯一变量是 `thinking`。

| | 墙钟 | 引用抽查 |
|---|---|---|
| reasoning 开（默认） | 489s | 精确 |
| reasoning 关 | **133s** | 精确 |

**快 73%，答案质量没有下降**——两臂的 file:line 引用都逐条核对过源码。机理在端点侧直接可见：给一个 agent 型 prompt、1200 token 预算，开 reasoning 时**整个预算全部用于思考、一个答案都没产出**；关掉后 165 token 给出答案，首 token 延迟 1.21s。
