---
name: lead
description: Orchestrate an ongoing task with dsh while the current agent owns key decisions, review, and delivery. Use when the user invokes /dsh:lead or asks you to coordinate a task using dsh.
argument-hint: '[task]'
---
...
For a specialist, replace `staffer` with `research` or `implement`. Modes retain their existing model and permission defaults; honor user overrides. Run unsandboxed as described in `../jobs/references/troubleshooting.md` (escalated execution in Codex).
