---
name: dsh-lead
description: Orchestrate an ongoing task with dsh while the current agent owns key decisions, review, and delivery. Use when the user invokes /skill:dsh-lead or asks you to coordinate a task using dsh.
---
...
Read `../dsh-jobs/SKILL.md` for result collection, cancellation, continuation, and recovery. This skill lives at `<plugin-root>/pi-skills/dsh-lead/SKILL.md`. Write the brief to a temporary file and call the shared companion:

```bash
node "<skill-dir>/../../companion/dsh-companion.mjs" staffer --prompt-file "<brief-path>"
```

For a specialist, replace `staffer` with `research` or `implement`. Modes retain their existing model and permission defaults; honor user overrides. Run unsandboxed as described in `../dsh-jobs/references/troubleshooting.md` (escalated execution in Codex).
